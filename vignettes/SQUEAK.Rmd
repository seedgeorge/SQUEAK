...
plot_modelled_curves(mixed_model,palette= colors)
```

```{r fig.height=4, fig.width=4, message=FALSE, warning=FALSE}
plot_modelled_slopes(mixed_model,palette= colors)
```
...
# curvforest = plot_modelled_curves(mixed_model,palette = colors) / plot_interaction_forest(mixed_model,palette = colors) 
# fig = slope + plot_spacer() + curvforest + plot_layout(ncol=3,widths = c(2,0.5,2))
# fig
```
